...
var SurfacePickUp = /*#__PURE__*/function () {
  ...
  _createClass(SurfacePickUp, [{
    ...
    value: function load(variantId) {
      var _this = this; // If no variant is available, empty element and quick-return
...
var dist_index_es = AsyncView;
; // CONCATENATED MODULE: ./source/scripts/sections/jsProduct.js

var Product = /*#__PURE__*/function () {
  _createClass2(Product, null, [{
    key: "load",
    value: function load(url) {
      return dist_index_es.load(url, // template name
      {
        view: 'quickshop'
      } // view name (suffix)
      );
  ...
  function Product($section) {
    ...
    _classCallCheck2(this, Product);

    // Add settings from schema to current object
    this._section = $section[0];
    this._data = Shopify.theme.getSectionData($section); // Ensure product media libraries are present
    ...
    var priceUIEl = $section[0].querySelector('[data-price-ui]');
    var surfacePickUpEl = $section[0].querySelector('[data-surface-pick-up]');
    this.variantSelection = $section[0].querySelector('[data-variant-selection]');

    if (this.variantSelection) {
      this.variantSelection.addEventListener('variant-change', function (event) {
        return _this4._switchVariant(event.detail.product, event.detail.variant, event.detail.state);
      });

      if (surfacePickUpEl) {
        this.surfacePickUp = new index_es(surfacePickUpEl);
        this.surfacePickUp.onModalRequest(function (contents) {
          Promise.all([_this4.variantSelection.getProduct(), _this4.variantSelection.getVariant()]).then(function (_ref4) {
            var _ref5 = _slicedToArray2(_ref4, 2),
                product = _ref5[0],
                variant = _ref5[1];

            var surfacePickUpModal = $section[0].querySelector('[data-surface-pick-up-modal]');
    ...
    if (this.variantSelection) {
      Promise.all([this.variantSelection.getProduct(), this.variantSelection.getVariant(), this.variantSelection.getState()]).then(function (_ref6) {
        var _ref7 = _slicedToArray2(_ref6, 3),
            product = _ref7[0],
            variant = _ref7[1],
            state = _ref7[2];

        return _this4._switchVariant(product, variant, state);
      });
    }
  }

  _createClass2(Product, [{
    key: "enableStickyScroll",
    value: function enableStickyScroll($stickyElement, $productGallery) {
    ...
    key: "enableLightbox",
    value: function enableLightbox($productGallery) {
      $productGallery.find('.product-gallery__link').fancybox({
        beforeClose: function beforeClose(instance) {
          ...
          setTimeout(function () {
            return $instanceGallery.fadeIn(100);
          }, 500);
        },
    ...
    key: "disableSlideshow",
    value: function disableSlideshow($section, selector) {
      var $slider;
    ...
    key: "enableSlideshow",
    value: function enableSlideshow(selector, settings) {
      // Define variables
      var $productGallery = selector;
      var $thumbnailProductGallery = $productGallery.closest('.product-gallery').find('.product-gallery__thumbnails');
      var $slides = $productGallery.find('.product-gallery__image');
      var $thumbnails = $thumbnailProductGallery.find('.product-gallery__thumbnail');

      function autoplayVideo(videoID, $slide) {
        // Compare id to player object and only play that video
        $.each(window.videoPlayers, function (_, player) {
          if (player.id === videoID) {
            player.play(); // On fullscreen toggle, focus back on the slide itself

            player.on('exitfullscreen', function () {
              return $slide.closest('.product-gallery').find('.product-gallery__thumbnails').focus();
            });
          }
        });
      }
      ...
      $thumbnailProductGallery.find('.gallery-cell:not(.product-gallery__thumbnail)').addClass('product-gallery__thumbnail'); // If custom settings available, use them otherwise take settings from product templates

      var _ref8 = settings || this._data,
          thumbnailsEnabled = _ref8.thumbnails_enabled,
          thumbnailsSliderEnabled = _ref8.enable_thumbnail_slider,
          thumbnailsPosition = _ref8.thumbnail_position,
          arrowsEnabled = _ref8.gallery_arrows,
          slideshowSpeed = _ref8.slideshow_speed,
          slideshowTransition = _ref8.slideshow_transition;
      ...
      $productGallery.on('ready.flickity', function () {
        $slides.each(function (index, slide) {
          ...
          var _window = window,
              videoPlayers = _window.videoPlayers;
          ...
          switch (mediaType) {
            ...
            case 'video':
              ...
              if (videoPlayers) {
                for (var _i2 = 0; _i2 < videoPlayers.length; _i2++) {
                  if (videoPlayers[_i2].id === videoID || videoPlayers[_i2].media.id === videoID) {
                    videoPlayers[_i2].loop = videoLooping;

                    if (!$(slide).hasClass('is-selected')) {
                      videoPlayers[_i2].keyboard = {
                        focused: true,
                        global: false
                      };
                    }
              ...
              if ($(slide).hasClass('is-selected')) {
                ...
                if (mediaType === 'model' && window.isScreenSizeLarge()) {
                  $(slide).on('mouseenter', function () {
                    return $productGallery.flickity('unbindDrag');
                  });
                  $(slide).on('mouseleave', function () {
                    return $productGallery.flickity('bindDrag');
                  });
                }
              }
          ...
          $(slide).keypress(function (event) {
            if (event.which === 13) {
              ...
              if (mediaType === 'model') {// If model container has class is-selected then play the model
      ...
      $productGallery.flickity({
        ...
        watchCSS: this._data.template === 'image-scroll' && !$productGallery.hasClass('js-gallery-modal'),
        ...
        $slides.each(function (index, slide) {
          ...
          if ($(slide).hasClass('is-selected')) {
            ...
            switch (mediaType) {
              ...
              case 'model':
                $.each(Shopify.theme.productMedia.models, function (_, model) {
                  return model.pause();
                });
                break;
      ...
      if (($sliderArrows || $thumbnails) && window.isScreenSizeLarge()) {
        ...
        $thumbnails.on('click', function (event) {
          ...
          $productGallery.on('settle.flickity', function () {
            ...
            if (mediaType === 'model') {
              ...
              $.each(sortedModels, function (_, model) {
                var $slide = $(model.container).parents('.product-gallery__image');
        ...
        $thumbnails.keypress(function (event) {
          ...
          if (event.which === 13) {
            ...
            var mediaType = $selectedSlide.data('media-type'); // Run video autoplay logic if featured media is a video
        ...
        $thumbnailProductGallery.find('.product-gallery__thumbnail').on('click', function (event) {
      ...
      $productGallery.on('settle.flickity', function () {
        return $productGallery.flickity('resize');
      });
        ...
        if (window.isScreenSizeLarge()) {
          if (thumbnailsPosition === 'right-thumbnails' || thumbnailsPosition === 'left-thumbnails') {
            ...
            $thumbnailProductGallery.on('settle.flickity', function () {
              return $thumbnailProductGallery.flickity('resize');
            });
            $(window).on('load', function () {
              return $thumbnailProductGallery.flickity('resize');
            });
    ...
    key: "findSelectedVariantImage",
    value: function findSelectedVariantImage() {
      ...
      $('[data-variant-selector]').on('selectedVariantChanged', function (event) {
        var $currentTarget = $(event.currentTarget);
    ...
    key: "scrollSelectedImage",
    value: function scrollSelectedImage(variant) {
      var headerHeight = 0;
    ...
    key: "_switchVariant",
    value: function _switchVariant(product, variant, state) {
      var _this5 = this;

      window.selectCallback(this._section.querySelector(".product-".concat(product.id)), product, variant, state);

      if (this.priceUI) {
        var formatter = function formatter(price) {
          return price === 0 ? Shopify.translation.free_price_text : Shopify.formatMoney(price, $('body').data('money-format'));
        };

        this.priceUI.load(product, {
          variant: variant,
          formatter: formatter,
          handler: function handler(priceUIFragment, p, _ref9) {
            var v = _ref9.variant;
            ...
            if (v && v.available) {
              var shopPayInstallmentsTemplate = _this5._section.querySelector('[data-shop-pay-installments-template] shopify-payment-terms');

              var shopPayInstallEl = null;
              ...
              if (_this5._data.display_savings && v.compare_at_price > v.price) {
                var span = document.createElement('span');
                span.classList.add('sale', 'savings');
                span.innerHTML = "".concat(Shopify.translation.product_savings, " ").concat(parseInt((v.compare_at_price - v.price) * 100 / v.compare_at_price, 10), "% (<span class=\"money\">").concat(formatter(v.compare_at_price - v.price), "</span>)");
            ...
            if (Shopify.theme.currencyConverter) {
              var moneyEls = priceUIFragment.querySelectorAll('.money');
      ...
      if (document.querySelector('.variants-mobile-only span.product__sku')) {
        document.querySelector('.variants-mobile-only span.product__sku').innerText = variant.sku;
      }
    }
  }]);
...
Shopify.theme.jsProduct = {
  ...
  relatedProducts: function relatedProducts() {
    $('.js-related-products-slider .products-slider').each(function (_, slider) {
      var $relatedSlider = $(slider);
      var slideData = {
      ...
      if (slideData.products_available < slideData.products_per_slide || slideData.products_limit < slideData.products_per_slide) {
        ...
        $relatedSlider.on('settle.flickity', function () {
          return $relatedSlider.flickity('resize');
        });
        $(window).on('load', function () {
          return $relatedSlider.flickity('resize');
        });
  ...
  initMatrixForm: function initMatrixForm() {
    var selector = '.product_form_list',
        $section = $('.product-form-container'),
        form = $(selector, $section),
        inputGroup,
        cb,
        formatValue; // Not Matrix

    if (form.length === 0) return; // Callback function to update the input field

    cb = function cb(target, value) {
      var vid = target.getAttribute('data-variant-id');
      document.querySelectorAll("input[data-variant-id='".concat(vid, "']")).forEach(function (e) {
        e.value = value > 0 ? value : '';
      }); // Update Tooltips

      if (target.dataset.variantQty <= 0) return;
      var text = 'In Stock';

      if (target.dataset.variantQty < value) {
        text = 'On Backorder';
      }

      document.querySelectorAll("b[data-vid='".concat(vid, "']")).forEach(function (e) {
        target.dataset.variantQty < value ? e.classList.remove('green') : e.classList.add('green');
        e.innerText = text;
      });
    ...
    inputGroup = $('.input-group', form); // Button Event

    inputGroup.find('button.minus-symbol').click(function (e) {
      var field = e.target.parentNode.querySelector('input'),
          value;
      if (!field) return;
    ...
    inputGroup.find('button.plus-symbol').click(function (e) {
      var field = e.target.parentNode.querySelector('input'),
          value;
      if (!field) return;

      if (field.dataset['policy'] === 'continue') {
        value = formatValue(field.value);
        value += 1;
        cb(field, value);
        return;
    ...
    inputGroup.find('input').on('blur change', debounce(function (e) {
      var field = e.target,
    ...
    var btns = $('.one-third.button--add-to-cart', $section);
    btns.click(function (e) {
      e.preventDefault(); // Disable btn
      ...
      debounce(function () {
        ...
        document.querySelectorAll('.input-group > input').forEach(function (input) {
          ...
          if (!qty || qty <= 0) return;
          data[input.dataset['variantId']] = {
            quantity: qty,
            id: parseInt(input.dataset['variantId']),
            properties: {
              price: input.dataset['variantPrice'],
              tier: input.dataset['tierName'],
        ...
        $.ajax({
          ...
          success: function success() {
            // Success Message
            Shopify.theme.jsAjaxCart.updateView(); // Update Cart Icon

            Shopify.theme.jsAjaxCart.showDrawer(); // enable btn
