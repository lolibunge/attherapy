...
Shopify.theme.jsSidebar = {
  init: function init() {
    var facetedFilterForm = document.querySelector('[data-faceted-filter-form]');

    if (facetedFilterForm) {
      facetedFilterForm.addEventListener('change', function (e) {
        if (e.target.type == 'number') return;
        facetedFilterForm.submit();
      });
    }

    var openSidebarBlocks = document.querySelectorAll('[data-sidebar-block__toggle="open"]');
